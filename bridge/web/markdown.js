  ...
  function render(value) {
    ...
    for(let i=0;i<lines.length;i++) {
      ...
      if(fence) {
        ...
        out.push(`<div class="code-block"><div class="code-head"><span>${escape(fence[2] || 'code')}</span><button class="copy-code" data-code="${escape(encodeURIComponent(code.join('\n')))}">Copy</button></div><pre><code>${escape(code.join('\n'))}</code></pre></div>`); continue;
      }
      ...
      if(line.includes('|') && delimiter.length && delimiter.every(s=>/^:?-+:?$/.test(s)) && delimiter.length===cells(line).length) {
        ...
        const row=(values, tag)=>'<tr>'+values.map((v,c)=>`<${tag} style="text-align:${delimiter[c]?.endsWith(':')?(delimiter[c].startsWith(':')?'center':'right'):'left'}">${inline(v)}</${tag}>`).join('')+'</tr>';
        out.push('<div class="table-scroll"><table><thead>'+row(head,'th')+'</thead><tbody>'+rows.map(r=>row(r,'td')).join('')+'</tbody></table></div>'); continue;
      }
