	...
	return {
		{
			...
			risk = "danger",
			needs = { "exec" },
			description = "Run bounded Luau on the local client. Captures print/warn, tables and all return values; waits for functions started with task.spawn/defer/delay. Stop or the deadline cancels managed tasks. Loop checkpoints yield automatically. Prefer a specific tool when one exists. This is not a security sandbox; dynamically loaded code and engine calls can bypass cooperative guards.",
			timeout = function(args) return execution.timeout(args) + 1 end,
			parameters = {
				type = "object",
				properties = {
					code = { type = "string", minLength = 1, maxLength = 256000, description = "The source to run. Use print() for output; return values or tables to report results." },
