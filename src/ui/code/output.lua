return function(env)
	...
	function M.new(parent)
		...
		local function render()
			...
			label.Text = "Run " .. run.id .. (doc and doc.revision ~= run.revision and ("\nRan revision " .. run.revision .. "; current revision " .. doc.revision) or "")
		...
		header.add("More", function(button) common.menu(button, "Output actions", { { label = "Copy output", value = "copy" }, { label = "Expand output", value = "expand" }, { label = "Latest run", value = "latest" } }, function(action)
			if action == "copy" then common.copy(label.Text) elseif action == "expand" then overlay.code({ title = "Run output", code = label.Text }) else selected = nil; render(); scroll.toBottom() end
